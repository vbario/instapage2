<template>
  <div class="landing-wrapper df fdc aic f1 w100p" @click="closeEmailCollection()">
    ...
    <ProductEditor v-if="showProductEditor"
                ...
                :productInitialType="productInitialType"
                @reloadStore="getStoreData"
    ...
    <CartIcon 
      v-if="sellerData.ownerId !== $store.getters['AUTH.uid']" 
      :brandColor="sellerData.brandColor || ''"
      :useCustomBrandColor="sellerData.useCustomBrandColor || ''"
      @showCart="showCart" />
    <OpenCart v-if="cartIsOpen"
      :shippingByOwner="shippingByOwner"
      :setShippingByOwner="setShippingByOwner"
      :storeOwnerUsername="$route.params.user"
      @showCart="showCart"
    ...
    <div class="w100p df fdc aic content-section">
      ...
      <Links v-if="(mode == 'link-in-bio')" :editingMyStore="editingMyStore"
            :loaded="loaded"
            :links="sellerData.links"
            :brandColor="sellerData.brandColor || ''"
            :useCustomBrandColor="sellerData.useCustomBrandColor || ''"
      ...
      <hr v-if="(mode == 'link-in-bio')" class="hr-x">
      <div v-if="(mode == 'link-in-bio') && blog" class="link-in-bio-grid-wrapper">
        <Blog />
      </div>
      <div v-else-if="(mode == 'link-in-bio') && page" class="link-in-bio-grid-wrapper">
        <Page />
      </div>
      <div v-else-if="(mode == 'link-in-bio')" class="link-in-bio-grid-wrapper">
        <div class="link-in-bio-grid">
          <div v-if="(sellerData.ownerId == $store.getters['AUTH.uid']) && (!sellerData.items || (sellerData.items.length == 0))" class="w100p df jcc aic">
            <div class="add-your-first df fdc jcc aic">
              <!-- <p class="text-dark tac">Add your first product</p> -->
              <p class="editor-heading">Add your first product or service</p>
              <ServiceTypeSelector :mt="16" @startNewProductWithType="startNewProductWithType" />
            </div>
          </div>
          <!-- <div
            v-if="scott() && product.type == 'link'"
            ...
            <div>
              <a class="curp" :href="product.url" targe="_blank">{{product.title}}</a>
            </div>
          </div> -->
          <div v-if="!(sellerData.items && sellerData.items.filter(v => v.public)[0]) && (!imTheOwner() || !editingMyStore)" class="w100p no-items">
            <p class="w100p store-coming-soon" :style="sellerData.brandColor ? 'color: ' + sellerData.brandColor + ';' : ''">Something awesome is coming soon!</p>  
          </div>
          <div v-for="(product, p) in sellerData.items"
            v-if="showProduct3(product)"          
            :class="cardClasses(product, p)"
            :style="cardStyles(product, p)"
            :draggable="editingMyStore"
            class="draggable"
            @click="cardClick(product, p)"
            @contextmenu.prevent="dragStart($event, 'contextmenu.prevent', p)"
            @touchend="dragEnd($event, 'touchend', p)"
            @pointerenter="itemDragEnter($event, 'pointerenter', p)"
            @dragstart="dragStart($event, 'start', p)"
            @dragover.prevent="handleDragB($event, p)"
            @dragenter="itemDragEnter($event, 'enter', p)"
            @dragleave="itemDragLeave($event, 'leave', p)"
            @drop="itemDrop($event, 'drop', p)"
            @dragend="dragEnd($event, 'end', p)">
            <!-- When Title -->
            ...
            <!-- When Grid Item -->
            <div class="w100p h100p df fdc jcc aic curp psorel" :id="p"
              v-if="showProduct2(product)" :class="{'archived-product': product.archive}">
              <p class=arhchived-text>Archived</p>
              <div if="product.productImage" class="w100p link-in-bio-grid-item pen" v-bind:style="backgroundStyle(product.productImage)">
              <div class="inactive-card" v-if="!product.public">
                <div class="w100p df fdc jcc aic">
                  <p class="item-status-text">Inactive</p>
                  <h4 @click="startOnboarding($event)" class="mini-button">Add Banking Info</h4>
                  <p class="item-status-description">to sell paid products</p>
                </div>
              </div>
                <ButtonWithMenu2
                  v-if="editingMyStore"
                  ...
                  @editItem="editItem(p)"
                  @archiveItem="archiveItem(p)"
                  @unArchiveItem="unArchiveItem(p)"
                  @deleteItem="deleteItem"/>
              </div>
          ...
          <div
            v-if="editingMyStore"
            :class="cardClasses({type:'extra'}, sellerData.items ? (sellerData.items.length) : 0)"
            :style="cardStyles({type:'extra'}, sellerData.items ? (sellerData.items.length) : 0)"
            class="draggable-end"
            @dragover.prevent
            @dragenter="itemDragEnter($event, 'enter', sellerData.items ? (sellerData.items.length) : 0)"
            @dragleave="itemDragLeave($event, 'leave', sellerData.items ? (sellerData.items.length) : 0)"
            @drop="itemDrop($event, 'drop', sellerData.items ? (sellerData.items.length) : 0)">
          </div>
        </div>
      </div>
    </div>
    <div class="w100p settings">
      <Settings v-if="mode == 'settings'"/>
      <More v-else-if="mode == 'more'"/>
      <Theme
        v-else-if="mode == 'theme'"
        :brandColor="sellerData.brandColor || ''"
        :useCustomBrandColor="sellerData.useCustomBrandColor || ''"
        :banner="sellerData.banner || {}"
        :hidePushableBranding="sellerData.hidePushableBranding"
        @reloadStore="getStoreData({}, true)"/>
      <Fulfillment v-else-if="mode == 'orders'"/>
    </div>
    <PushableBranding v-if="!sellerData.pro && !sellerData.hidePushableBranding" />
...
import Blog from '@/components/store/Blog'
...
export default {
  ...
  methods: {
    ...
    scott () {
      return this.$route.params.user && (this.$route.params.user.toLowerCase() == 'scottrao')
    },
    imTheOwner () {
      let myUid = this.$store.getters['AUTH.uid']
      return myUid == this.sellerData.ownerId
    },
    startNewProductWithType (data) {
      console.log('startNewProductWithType', data)
      // type
      // id == new

      // show ProductEditor
      this.productInitialType = data.type
      this.openProductEditor('new')
      setTimeout(() => {
        this.productInitialType = false
      }, 500)
    ...
    productPriceText (product) {
      ...
      if ((type == 'digital-download') || (type == 'custom-service')) {
        // !! add accounting for sale price
        if (product.freeProduct) {
          return 'free'
        }
        return product.price ? '$' + product.price : 'free'
      } else if (type == 'physical-product') {
        let minSizePrice = 9999999999999999999999999999999999999999999999999999
        for (let s in product.sizes) {
          let size = product.sizes[s]
          if (size.freeProduct) {
            minSizePrice = 'free'
          } else if (parseFloat(size.price) < minSizePrice) {
            minSizePrice = parseFloat(size.price)
          }
        }
        return 'from ' + (minSizePrice == 'free' ? '' : '$') + minSizePrice
      } else if (type == 'calendar-event') {
        let minSizePrice = 9999999999999999999999999999999999999999999999999999
        for (let s in product.appointmentLengths) {
          let size = product.appointmentLengths[s]
          if (size.freeProduct) {
            minSizePrice = 'free'
          } else if (parseFloat(size.price) < minSizePrice) {
            minSizePrice = parseFloat(size.price)
        ...
        return 'from ' + (minSizePrice == 0 ? 'free' : '$' + minSizePrice)
    ...
    editItem (i) {
      ...
      console.log('edit item', i, this.sellerData.items[i].productId)
      let products = this.$store.getters['AUTH.myProductsList']
      console.log('products', products)
      let product = products.filter(v => v.uniqueProductId == this.sellerData.items[i].productId)[0]
      console.log('product', product)
      this.showProductEditor = product.internalId
    },
    deleteItem (i) {
    ...
    archiveItem (i) {
      console.log('archiveItem', i, this.sellerData.items[i].key)
      // this.showTitleEditor = i
      let itemKey = this.sellerData.items[i].key
      if (itemKey) {
        this.archiveItemInStore(itemKey)
      }
    },
    unArchiveItem (i) {
      console.log('unArchiveItem', i, this.sellerData.items[i].key)
      // this.showTitleEditor = i
      let itemKey = this.sellerData.items[i].key
      if (itemKey) {
        this.unArchiveItemInStore(itemKey)
      }
    },
    ...
    archiveItemInStore (productId) {
      this.$store.dispatch('AUTH.archiveItemInStore', {
        productId,
        archive: true
      }).then((res) => {
        console.log('archiveItemInStore result', res)
        this.getStoreData()
      }).catch((error) => {
      })
    },
    unArchiveItemInStore (productId) {
      this.$store.dispatch('AUTH.archiveItemInStore', {
        productId,
        archive: false
      }).then((res) => {
        console.log('unArchiveItemInStore result', res)
        this.getStoreData()
      }).catch((error) => {
      })
    },
    ...
    cardStyles (product, p) {
      ...
      // if (!product.public && (product.type !== 'text') && !this.scott()) {
    ...
    showProduct3 (product) {
      if (product.type == 'text') {
        return true
      }
      if (product.archive || !product.public) {
        return this.editingMyStore
      }
      return true
    },
    ...
    getStoreData (params, useMyUsername) {
      console.log('params', params)
      let scrollLinksToEnd = params ? params.scrollLinksToEnd : false
      this.$store.dispatch('AUTH.getStoreData', {
        sellerUsername: useMyUsername ? this.$store.getters['AUTH.myInfo'].displayName.displayName : this.$route.params.user
      }).then((res) => {

        if (res.retry) {
          setTimeout(() => {
            this.getStoreData(params, useMyUsername)
          }, 500)
          return false
        }
    ...
    getMyStoreData () {
      if (this.$store.getters['AUTH.myInfo'] && this.$store.getters['AUTH.myInfo'].displayName) {
        this.getStoreData({scrollLinksToEnd: false}, true)
      } else {
        setTimeout(() => {
          this.getMyStoreData()
        }, 500)
  ...
  watch: {
    ...
    //     console.log('showProductEditor', show)
    //     if (show) {
  ...
  created () {
    ...
    if (
      (routePage == 'ThemePage') ||
      (routePage == 'OrdersPage')
    ) {
      this.editingMyStore = true
    }
  },
  mounted () {
    if (this.mode == 'link-in-bio') {
