  ...
  <div class="button-with-menu-wrapper df aic jcc">
    ...
    <div v-if="menuIsOpen" class="button-dropdown-menu"
      ...
      <div class="button-dropdown-menu-inner">
        <div class="button-dropdown-menu-item" @click="editItem($event)">
          <img class="button-menu-item" src="../../../static/brand-icons/edit.svg">
          <p>Edit</p>
        </div>
        <div v-if="!archive" class="button-dropdown-menu-item" @click="archiveItem($event)">
          <img class="button-menu-item" src="../../../static/brand-icons/archive.svg">
          <p>Archive</p>
        </div>
        <div v-else class="button-dropdown-menu-item" @click="unArchiveItem($event)">
          <img class="button-menu-item" src="../../../static/brand-icons/archive.svg">
          <p>Un-Archive</p>
        </div>
...
export default {
  name: 'ButtonWithMenu2',
  data () {
    return {
      menuIsOpen: false
    }
  },
  props: ['editingMyStore', 'p', 'archive'],
  components: {
  ...
  methods: {
    ...
    archiveItem (event) {
      event && event.stopPropagation()
      console.log('archiveItem', this.p)
      this.$emit('archiveItem', {i: this.p})
      this.menuIsOpen = false
    },
    unArchiveItem (event) {
      event && event.stopPropagation()
      console.log('unArchiveItem', this.p)
      this.$emit('unArchiveItem', {i: this.p})
      this.menuIsOpen = false
    },
    editItem (event) {
      event && event.stopPropagation()
      console.log('editItem', this.p)
      this.$emit('editItem', {i: this.p})
      this.menuIsOpen = false
    },
