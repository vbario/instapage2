  ...
  <div class="add-link-outer" @click="closeLinkEditor()">
    <div class="add-link-wrapper">
      <div class="w100p add-link-box" @click="stopProp($event)">
        ...
        <h3 class="w100p add-link-box-title">Contact</h3>
        ...
        <div class="w100p df fdc aifs mb_x">
          <h5 class="add-link-box-heading"
            :class="{'field-error-text': error.hasError && (error.field == 'messageText')}">
            Message
          </h5>
          <textarea
            v-model="messageText"
            ...
            class="add-link-box-textarea"
            :class="{'field-error-exists': error.hasError && (error.field == 'messageText')}"
            name=""></textarea>
          <p class="field-error-text" v-if="error.hasError && (error.field == 'messageText')">{{error.text}}</p>
        </div>
        <div class="w100p actions df fd jcsb aic" :class="{'waiting-for-action': saving}">
          ...
          <div class="f1"></div>
          <h4 class="action-button button-primary" @click="sendMessge()">
            <img
              v-if="saving"
  ...
  methods: {
    ...
    validateData () {
      ...
      if (!this.isValidEmail(this.myEmail)) {
        ...
        this.setError('messageText', 'Please enter a message.')
        return false
      } 
      return true
    },
    ...
    sendMessge () {
      ...
      this.saving = true
      this.$store.dispatch('AUTH.sendMessge', {
        email: this.myEmail,
        ...
        this.saving = false
        this.setError(false)
        this.$emit('flashSubmittedContactForm')
      })
    },
    closeLinkEditor () {
      this.$emit('closeContactUsModal')
    },
    stopProp (event) {
